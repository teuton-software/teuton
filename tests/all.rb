#!/usr/bin/env ruby

require 'minitest/autorun'

require_relative 'application_test'
require_relative 'case_manager/case_test'
require_relative 'case_manager/case/config_test'
require_relative 'case_manager/case/result_test'
require_relative 'case_manager/case/case_model/target_model_test'
require_relative 'case_manager/case/case_model/group_model_test'
require_relative 'case_manager/case/case_model/case_model_test'
require_relative 'project/configfile_reader_test'
require_relative 'project/project_test'
# require_relative 'rubocop_test'
