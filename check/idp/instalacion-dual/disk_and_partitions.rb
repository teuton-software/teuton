...
task :disk_size do
  ...
  goto :suse1, :exec => "lsblk |grep disk| grep sda| grep #{size}|grep G| wc -l"
  expect result.eq(1)
end
...
task :partitions_size_and_type do
  ...
  partitions.each_pair do |key,value|
    ...
    goto :suse1, :exec => "lsblk |grep part| grep #{key}| grep #{value[0]}| wc -l"
    expect result.eq(1)

    target "Partition #{key} size <#{value[1]}>"
    goto :suse1, :exec => "lsblk |grep part| grep #{key}| tr -s ' ' ':'| cut -d ':' -f 5"
  ...
  partitions.each_pair do |key,value|
    ...
    goto :suse1, :exec => "df -hT | grep #{key}| grep #{value[2]}|wc -l"
    expect result.eq(1)
  end
end
