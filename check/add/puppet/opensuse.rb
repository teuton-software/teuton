...
task "OpenSUSE external configurations" do
  ...
  set :host3_ip, "172.18." + get(:number).to_i.to_s + ".100"

  target "ping to <"+get(:host3_ip)+">"
  goto :localhost, :exec => "ping #{get(:host3_ip)} -c 2"
  expect result.find!('64 bytes from').ge(1)

  target "SSH port 22 on <"+get(:host3_ip)+"> open"
  goto :localhost, :exec => "nmap #{get(:host3_ip)} -Pn"
...
task "OpenSUSE student configurations" do
  ...
  goto :host3, :exec => "cat /etc/passwd"
